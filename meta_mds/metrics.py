...
def remove_files_proc():
    #exclui arquivos
    os.remove(DISTANCES['D_high_list'])
    os.remove(DISTANCES['D_high_matrix'])  
...
def eval_pq_metrics(**kwargs):
    ...
    #exclui arquivos
    os.remove(DISTANCES[id_run]['D_low_list'])
    os.remove(DISTANCES[id_run]['D_low_matrix'])
    os.remove(DISTANCES[id_run]['Q'])
