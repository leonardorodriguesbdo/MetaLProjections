...
def validacao_Spearman(rk_predito, rk_padrao):
    ...
    return res_asymptotic.statistic ,res_asymptotic.pvalue, res_exact.pvalue
...
data = df_base_con.loc[df_base_con['BD'] == 'ads']
print(data)

'''# separa da base de conhecimento a parte dos rankings
df_base_con_ranking = df_base_con[['IDMAP', 'IPCA', 'LAMP', 'LMDS', 'MDS', 'PBC', 'TSNE', 'UMAP']] #df_base_con[[31,32,33,34,35,36,37,38]]
df_base_con_ranking = df_base_con_ranking.astype(float)
#print(df_base_con_ranking)
#ranking_padrao = rk.ranking_padrao(df_base_con_ranking)'''
ranking_padrao = rk.ranking_padrao(base_conhecimento_ranking(df_base_con))
#print(ranking_padrao)

'''# separa da base de conhecimento a parte dos meta-atributos
df_base_con_meta_atrib = df_base_con
df_base_con_meta_atrib = df_base_con_meta_atrib.drop(columns=['BD','MA1','IDMAP', 'IPCA', 'LAMP', 'LMDS', 'MDS', 'PBC', 'TSNE', 'UMAP']) #columns=[0,1,31,32,33,34,35,36,37,38])
df_base_con_meta_atrib.reset_index(inplace=True, drop=True)
...
d = np.random.uniform(0.,1.,size=(1,29))
print(d)
neigh_dist, neigh_ind = neigh.kneighbors(d)
print(neigh_ind.ravel())
...
#print(teste.dtypes)
#print(rdf.T)
ranking_predito = rk.ranking_predito(df_rk.T)'''

d = base_conhecimento_atributos(data)
print(d)
k = 7
ranking_predito = knn_mds(df_base_con, k, d)
...
'''print('Coeficiente de Correlação Spearman: ',stats.spearmanr(ranking_predito.squeeze(), ranking_padrao.squeeze()))

res_exact = stats.permutation_test((ranking_predito.squeeze(),), statistic, permutation_type='pairings')
res_asymptotic = stats.spearmanr(ranking_predito.squeeze(), ranking_padrao.squeeze()) 
print('Probabilidade de ser casual: ', res_exact.pvalue , res_asymptotic.pvalue)  # asymptotic pvalue is too low'''
coef_spearman, res_exact, res_asymptotic = validacao_Spearman(ranking_predito, ranking_padrao)
print('Coeficiente de Correlação Spearman: ', coef_spearman)
